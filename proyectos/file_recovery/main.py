...
""",
    ...
    p.add_argument("--hint-path", action="append", metavar="RUTA", dest="hint_paths",
                   help="Carpeta donde estaban los archivos (se puede repetir). "
                        "Limita el carving a esa zona del disco y es mucho más rápido. "
                        "Ej: --hint-path \"C:\\Users\\paast\\Videos\"")
    p.add_argument("--hint-user", metavar="USUARIO",
                   help="Añade automáticamente Videos, Descargas, Escritorio, "
                        "Papelera y Temp del usuario indicado. "
                        "Ej: --hint-user paast")
...
def main():
    ...
    # ── Rutas de pista para carving focalizado ────────────────────────────────
    hint_paths = list(args.hint_paths) if args.hint_paths else []
    if args.hint_user:
        from cluster_locator import build_hint_paths
        hint_paths.extend(build_hint_paths(args.hint_user))
